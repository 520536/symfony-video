...
class VideoRepository extends ServiceEntityRepository
{
    ...
    public function findByChildIds(array $value, int $page, ?string $sort_method)
    {
        ...
        {
            $dbquery =  $this->createQueryBuilder('v')
            ->addSelect('COUNT(l) AS HIDDEN likes') // bez hidden zwróci array: count + entity hidden减少querry次数
            ->leftJoin('v.usersThatLike', 'l')
            ->andWhere('v.category IN (:val)')
            ->setParameter('val', $value)
            ->groupBy('v')
    ...
    {
        $terms = array_unique(explode(' ', $query));    //explode() 函数把字符串打散为数组。

        return array_filter($terms, function ($term) {   //如果搜索匹配的字符少于2个则按照最多的匹配
            return 2 <= mb_strlen($term);   //mb_strlen()获取字符串长度
        });
    }
